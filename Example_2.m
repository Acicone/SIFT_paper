...
hold on ;
% plot(time, if1, 'k', 'linewidth', 2) ; set(gca, 'fontsize', 30) ; 
% plot(time, if2, 'k', 'linewidth', 2) ; %axis([0 12 0 18]) ; 
...
opt = Settings_ALIF('ALIF.NIMFs',1,'plots',0,'saveplots',0,'ALIF.xi',1.1,'ALIF.delta',10^-6);
...
plot(time,real(IMT_SST_nWin(1,:))-s1'-2,'k','linewidth',2)
plot(time,IMT(2,:)-s1'-4,'r','linewidth',2)
...
plot(time,real(IMT_SST_nWin(2,:))-s2'-2,'k','linewidth',2)
plot(time,IMT(4,:)-[s2']-4,'r','linewidth',2)
lg=legend('BPF','SST','SST after tuning','SIFT');
set(lg,'Interpreter','latex')
axis xy ; set(gca,'fontsize', 30) ; 
axis([1 15 -5 8])
